...
## Актуализация от 2026-09-06: Telegram-аккаунты администраторов

Административные Telegram-уведомления переведены с одного статического chat ID на
персональные привязки web-администраторов. В `Настройки -> Уведомления Telegram` каждый
администратор может получить одноразовую команду, подключить личный чат с ботом,
отправить тестовое сообщение и безопасно отключиться. Система поддерживает несколько
одновременных администраторов и создает независимый outbox intent для каждого.

Коды действуют ограниченное время, в БД хранятся только как HMAC, endpoint доступен
только доверенному bot service client, принимает привязку только из private chat и
ограничен rate limit. У каждой привязки есть `generation`: ожидающие сообщения старой
версии отбрасываются после rebind/unbind. При удалении, деактивации или снятии verification
активные одноразовые коды отзываются, а Telegram identity очищается Eloquent observer и
PostgreSQL lifecycle-trigger. Tombstone остается, поэтому старый `TELEGRAM_ADMIN_CHAT_ID` не может
самопроизвольно включиться снова. Env ID остается только bootstrap fallback для чистой
установки до первого bind.
...
`knex seed:run` не является рабочей командой: папки `seeds` нет. Схема создаётся миграциями, а данные для локальной разработки должны поступать отдельным обезличенным набором вне Git.
...
- запуск миграций и, при необходимости, импорт отдельно переданного обезличенного набора данных;
- пример `.env`;
...
- legacy SQL-дампы удалены из актуального дерева, потому что содержали пользовательские данные; схема воспроизводится миграциями.
...
- пользователь сначала выбирает байки или автомобили, затем сезон;
- универсальные картинки находятся в
  `images/prices/<type>_<season>.PNG`, локализованные варианты имеют приоритет;
- изображения остаются только UI-материалом, расчет цены выполняет Laravel.
...
- Не добавлять SQL-дампы с пользовательскими данными; изменения схемы оформлять только миграциями.
