...
- Универсальные финальные прайсы байков и автомобилей для `high`/`middle`/`low`
  предоставлены и подключены. При необходимости архитектура поддерживает отдельные RU/EN
  варианты без изменения обработчика.
