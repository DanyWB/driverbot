...
На текущей машине есть PHP 8.3.6, Composer 2.6.6 и Node.js 22.20.0. Перед созданием
Laravel нужно исправить локальный `sys_temp_dir` Composer. Docker сейчас отсутствует,
поэтому первый локальный запуск допускается через OSPanel/PostgreSQL/Redis. Конфигурация
не должна зависеть от OSPanel и должна иметь документированные env-переменные.
...
Текущий Node.js-проект временно находится в корне. Его перенос в `bot/` выполняется
отдельным механическим коммитом после фиксации baseline, чтобы не смешивать перемещение
файлов с изменением поведения.
