...
class VehiclePricingController extends Controller
{
    ...
    public function update(
        ...
        DB::transaction(function () use ($request, $vehicle, $pricing, $audit, $data, $oldMatrix, $wasVisible): void {
            $updated = $pricing->replaceMatrix($vehicle, $data['prices'], $data['enabled']);
            $audit->record(
                $this->admin($request),
                $updated,
                'vehicle.pricing_updated',
                ['matrix' => $oldMatrix, 'is_visible_for_booking' => $wasVisible],
                ['matrix' => $pricing->matrix($updated), 'is_visible_for_booking' => (bool) $updated->is_visible_for_booking],
