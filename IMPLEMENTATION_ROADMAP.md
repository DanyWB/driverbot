...
- два клика по свободным датам одной техники -> подсветка периода и боковая форма
  ручной брони;
...
- после создания timeline обновляется без потери периода и фильтров;
...
### Промежуточный контроль после этапа 8

2026-07-22 выполнена полная сверка этапов 0-8 с архитектурой, ТЗ и фактическим UI.
Подтвержденные дефекты временных интервалов, batch API, операций с ценой, admin auth,
CSV/files, Redis-session и CI исправлены и покрыты regression tests. Полный отчет,
включая намеренно незавершенный scope, находится в
`INTERMEDIATE_AUDIT_2026-07-22.md`. Архитектурный вектор не изменился; следующий
этап остается этапом 9.

## 12. Этап 9. Автоматизация и уведомления

Оценка: 8-10 часов.

Статус: выполнен 2026-07-22. Delivery pipeline, шаблоны, scheduler, retry/backoff,
reminders, monitoring, housekeeping и эксплуатационные команды описаны в
...
Статус: инженерная и локально проверяемая часть выполнена 2026-07-22. Production acceptance
остается открытым до развертывания на реальном сервере, установки systemd/Nginx/PHP-FPM,
передачи runtime secrets, финального контента и приемки заказчиком. Фактический результат,
локальная restore/staging-репетиция и внешние gate описаны в
`STAGE_10_RELEASE_HARDENING.md` и `RELEASE_ACCEPTANCE_CHECKLIST.md`.
...
Подготовить production-сервер и пройти внешний acceptance gate этапа 10. Указать реальные
`APP_URL`, SMTP, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_ADMIN_CHAT_ID`,
`MANAGER_TELEGRAM_USERNAME`, утвержденную версию условий, создать admin/service client,
подтвердить финальный каталог и загрузить primary photo. Затем выполнить deployment, online
Telegram smoke, production-like load/rollback rehearsal и заказчиковую staging-приемку по
