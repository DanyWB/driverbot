...
module.exports = async (ctx) => {
  ...
  if (!rentals || rentals.length === 0) {
    return ctx.reply("⚠️ У вас нет байков в процессе аренды.");
  }
  try {
  ...
  keyboard.push([{text: "🔙 Назад", callback_data: "book:add_rental"}]);

  return ctx.reply("🗑 Выберите байк, который хотите удалить из аренды:", {
    reply_markup: {
