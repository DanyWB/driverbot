...
Текущий статус реализации: этапы 0-6 roadmap закрыты. Созданы Laravel foundation,
целевая доменная схема, импорт техники и цен, транзакционный booking/availability core
и основная web-админка бронирований с timeline занятости. Администратор уже может создать бронь без
Telegram, проверить цену и доступность, выполнить статусные действия, изменить даты
или финальную цену, увидеть историю и открыть календарь по всей технике. Из свободной
ячейки создается предзаполненная ручная бронь, а переход в карточку сохраняет период и
фильтры. Legacy-бот пока остается на старой БД до этапа Bot API. Следующий этап - CRUD
техники/категорий, фото, редактор тарифов, клиенты и CSV.
